...
export default function SprintBoardRefactor() {
  ...
  const [selectedSprint, setSelectedSprint] = useState<"Opex" | "Backlog" | "Tutti">("Opex");
  ...
  const { data, error } = await supabase
    ...
    // AGGIUNTO: Filtro per tipo di attività
  ...
  if (data) {
    setTickets(data.map((t: any) => ({
      ...t,
      // Se il database non ha la colonna columnId, la calcoliamo dallo stato
...
loadData();
  ...
  const filteredTickets = useMemo(() => {
    return tickets.filter((t) => {
      ...
      const matchesSearch = !searchQuery || t.titolo.toLowerCase().includes(search) || (t.n_tag || "").toLowerCase().includes(search);
        ...
        <div className="mb-8 space-y-4">
          <div className="flex flex-col xl:flex-row xl:items-center xl:justify-between gap-4">
            <div className="flex items-center gap-4">
              ...
              <div>
                <h1 className="text-3xl font-black tracking-tighter text-gray-900 uppercase">Opex Board</h1>
