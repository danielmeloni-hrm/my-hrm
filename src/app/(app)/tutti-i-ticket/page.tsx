...
export default function StoricoTicketPage() {
  ...
  if (loading) {
    return (
      <AppPage
        title="Database Attività"
        subtitle="Inizializzazione sistema"
        maxWidth="full"
      >
        <AppCard className="flex min-h-[420px] flex-col items-center justify-center">
          <div className="mb-4 h-10 w-10 animate-spin rounded-full border-[3px] border-slate-100 border-t-blue-600" />
  ...
  if (error) {
    return (
      <AppPage title="Database Attività" subtitle="Errore caricamento dati" maxWidth="full">
        <AppCard className="text-sm font-semibold text-red-600">Errore: {error}</AppCard>
      </AppPage>
    );
  }
  ...
  return (
    <AppPage
      title="Database Attività"
      subtitle={`Numero Attività = ${filteredTickets.length}`}
      maxWidth="full"
      actions={
        <AppCard className="flex flex-wrap items-center gap-6 p-2">
  ...
  >
    ...
    {selectedRicorsivo === '' ? 'Tutti' : selectedRicorsivo === 'si' ? 'Sì' : 'No'}
            ...
            <AppButton
              type="button"
              variant={filterAttenzioneBusiness ? 'primary' : 'secondary'}
            ...
            <AppButton
              ...
              aria-label="Impostazioni colonne"
            >
              <Settings2 size={20} />
            </AppButton>
        </AppCard>
      ...
      {showConfig && (
        <div className="mb-8 animate-in fade-in slide-in-from-top-2">
          ...
          >
            ...
            >
              <AppCard className="grid grid-cols-2 gap-2 md:grid-cols-4 lg:grid-cols-6">
                {columnOrder.map((col) => (
                  <SortableColumnItem
                    key={col.id}
                    col={col}
