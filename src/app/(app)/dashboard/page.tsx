...
export default function GeneralDashboard() {
  ...
  const getZoomStyles = () => {
    ...
    return {
      colWidth: columnWidths[zoomLevel - 1],
      padding: cardPaddings[zoomLevel - 1],
      title: titleSizes[zoomLevel - 1]
    }
  }
  ...
  const handleUpdateTicket = async (id: string, updates: any) => {
    ...
    setSelectedTicket((prev: any) => prev?.id === id ? { ...prev, ...updates } : prev)

    const { error } = await supabase.from('ticket').update(updates).eq('id', id)
    if (error) console.error('Errore salvataggio:', error)
  }

  const updateSelectedTicketField = (field: string, value: any) => {
    setSelectedTicket((prev: any) => prev ? { ...prev, [field]: value } : prev)
  }

  const saveSelectedTicketField = async (field: string) => {
    if (!selectedTicket?.id) return
    await handleUpdateTicket(selectedTicket.id, { [field]: selectedTicket[field] })
  }
  ...
  const fetchBoardData = useCallback(async () => {
    ...
    if (selectedSprint !== 'Tutti') query = query.eq('sprint', selectedSprint)
    ...
    setLoading(false)
  }, [selectedSprint, filterMe, supabase, fetchUserPreferences])
  ...
  return (
    <div className="min-h-screen bg-[#FBFBFB] px-4 pt-8 pb-24 relative overflow-x-hidden">
      {showSettings && (
        <div className="fixed inset-0 z-[60] flex items-center justify-center bg-black/40 backdrop-blur-sm">
          <div className="bg-white p-6 rounded-2xl shadow-xl border border-gray-100 w-full max-w-sm">
                ...
                <button
                  ...
                  onClick={async () => {
                    const next = visibleColumns.includes(col.id)
                      ? visibleColumns.filter(i => i !== col.id)
                    ...
                    const { data: { user } } = await supabase.auth.getUser()
                    if (user) {
                      await supabase.from('profili').update({ kanban_columns: next }).eq('id', user.id)
                    }
                  }}
            ...
            <button
              onClick={() => setShowSettings(false)}
              className="w-full mt-4 bg-gray-900 text-white py-3 rounded-xl font-black uppercase text-[9px] tracking-widest"
            >
              Chiudi
            </button>
          </div>
      ...
      {selectedTicket && (
        ...
        >
          <div
            className="w-full max-w-4xl bg-white rounded-2xl shadow-2xl border border-gray-100 overflow-hidden max-h-[90vh] flex flex-col"
            ...
            <div className="flex items-start justify-between px-6 py-5 border-b border-gray-100">
              <div className="pr-4 w-full">
                ...
                <textarea
                  ...
                  onChange={(e) => updateSelectedTicketField('titolo', e.target.value)}
                  onBlur={() => saveSelectedTicketField('titolo')}
                  className="w-full text-xl font-black text-gray-900 italic leading-tight outline-none border-none resize-none bg-transparent"
            ...
            <div className="p-6 overflow-y-auto space-y-5">
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                <div className="bg-gray-50 rounded-xl p-4">
                  <p className="text-[10px] font-black uppercase text-gray-400 mb-2">Cliente</p>
                  <p className="text-sm font-bold text-gray-900">{selectedTicket.clienti?.nome || 'N/D'}</p>
                </div>

                <div className="bg-gray-50 rounded-xl p-4">
                  <p className="text-[10px] font-black uppercase text-gray-400 mb-2">Applicativo</p>
                  <input
                    value={selectedTicket.applicativo || ''}
                    onChange={(e) => updateSelectedTicketField('applicativo', e.target.value)}
                    onBlur={() => saveSelectedTicketField('applicativo')}
                    className="w-full text-sm font-bold text-gray-900 bg-white border border-gray-200 rounded-xl px-3 py-2 outline-none focus:border-blue-400"
                    placeholder="Applicativo"
                  />
                </div>

                <div className="bg-gray-50 rounded-xl p-4">
                  <p className="text-[10px] font-black uppercase text-gray-400 mb-2">Stato</p>
                  <select
                    value={selectedTicket.stato || ''}
                    onChange={(e) => {
                      updateSelectedTicketField('stato', e.target.value)
                    }}
                    onBlur={() => saveSelectedTicketField('stato')}
                    className="w-full text-sm font-bold text-gray-900 bg-white border border-gray-200 rounded-xl px-3 py-2 outline-none focus:border-blue-400"
                  >
                    {allBoardColumns.map(col =>
                ...
                <div className="bg-gray-50 rounded-xl p-4">
                  <p className="text-[10px] font-black uppercase text-gray-400 mb-2">Sprint</p>
                  <select
                    value={selectedTicket.sprint || ''}
                    onChange={(e) => updateSelectedTicketField('sprint', e.target.value)}
                    onBlur={() => saveSelectedTicketField('sprint')}
                    className="w-full text-sm font-bold text-gray-900 bg-white border border-gray-200 rounded-xl px-3 py-2 outline-none focus:border-blue-400"
                  >
                ...
                <div className="bg-gray-50 rounded-xl p-4">
                  <p className="text-[10px] font-black uppercase text-gray-400 mb-2">Tag</p>
                  <input
                    value={selectedTicket.n_tag || ''}
                    onChange={(e) => updateSelectedTicketField('n_tag', e.target.value)}
                    onBlur={() => saveSelectedTicketField('n_tag')}
                    className="w-full text-sm font-bold text-gray-900 bg-white border border-gray-200 rounded-xl px-3 py-2 outline-none focus:border-blue-400"
                    placeholder="Tag"
                  />
                </div>
                ...
                <div className="bg-gray-50 rounded-xl p-4">
                  <p className="text-[10px] font-black uppercase text-gray-400 mb-2">Ultimo Ping</p>
                  <p className="text-sm font-bold text-gray-900">
                    {selectedTicket.ultimo_ping ? formatDateShort(selectedTicket.ultimo_ping) : 'N/D'}
                  </p>
                </div>
              </div>
              ...
              <div className="bg-white border border-gray-100 rounded-2xl p-5">
                <div className="flex items-center justify-between mb-3">
                  <p className="text-[10px] font-black uppercase text-gray-400">Avanzamento</p>
                ...
                <input
                  ...
                  onChange={(e) => updateSelectedTicketField('percentuale_avanzamento', Number(e.target.value))}
                  onMouseUp={() => saveSelectedTicketField('percentuale_avanzamento')}
                  onTouchEnd={() => saveSelectedTicketField('percentuale_avanzamento')}
                  className="w-full"
                />
              ...
              <div className="bg-gray-50 rounded-2xl p-5">
                <p className="text-[10px] font-black uppercase text-gray-400 mb-2">Descrizione</p>
                <textarea
                  rows={8}
                  value={selectedTicket.descrizione || ''}
                  onChange={(e) => updateSelectedTicketField('descrizione', e.target.value)}
                  onBlur={() => saveSelectedTicketField('descrizione')}
                  className="w-full text-sm text-gray-700 leading-relaxed whitespace-pre-wrap bg-white border border-gray-200 rounded-xl px-4 py-3 outline-none resize-none focus:border-blue-400"
                  placeholder="Scrivi una descrizione..."
                />
              </div>

              <div className="bg-gray-50 rounded-2xl p-5">
                <p className="text-[10px] font-black uppercase text-gray-400 mb-2">Note</p>
                <textarea
                  rows={6}
                  value={selectedTicket.note || ''}
                  onChange={(e) => updateSelectedTicketField('note', e.target.value)}
                  onBlur={() => saveSelectedTicketField('note')}
                  className="w-full text-sm text-gray-700 leading-relaxed whitespace-pre-wrap bg-white border border-gray-200 rounded-xl px-4 py-3 outline-none resize-none focus:border-blue-400"
              ...
              <div className="flex items-center justify-between gap-4">
                <button
                  onClick={async () => {
                    const newValue = !selectedTicket.in_lavorazione_ora
                    updateSelectedTicketField('in_lavorazione_ora', newValue)
                    await handleUpdateTicket(selectedTicket.id, { in_lavorazione_ora: newValue })
                  }}
                ...
                >
                  {selectedTicket.in_lavorazione_ora ? 'In lavorazione ora' : 'Segna in lavorazione'}
                </button>

                <button
                  onClick={() => setSelectedTicket(null)}
                  className="px-5 py-3 bg-gray-900 text-white rounded-xl font-black uppercase text-[10px] tracking-widest hover:bg-blue-600 transition-all"
                >
                  Chiudi
                </button>
              </div>
          ...
          <div className="flex flex-col md:flex-row justify-between items-center gap-4">
            ...
            <div className="flex flex-wrap items-center gap-2">
              ...
              <button
                ...
                className={`flex items-center gap-1.5 px-4 py-2.5 rounded-xl text-[9px] font-black uppercase border transition-all ${
                  filterOnlyExpired ? 'bg-red-500 text-white border-transparent' : 'bg-white text-gray-500 border-gray-100'
                }`}
              >
              ...
              <button
                ...
                className={`flex items-center gap-1.5 px-4 py-2.5 rounded-xl text-[9px] font-black uppercase border transition-all ${
                  filterMe ? 'bg-blue-600 text-white border-transparent' : 'bg-white text-gray-500 border-gray-100'
                }`}
              >
              ...
              <button
                ...
                className="p-3 bg-white text-gray-400 rounded-xl border border-gray-100 hover:text-blue-600 transition-all shadow-sm"
              >
                <Settings2 size={16} />
              </button>
            </div>
                ...
                >
                  <div className="px-1">
                    ...
                    <div className={`flex items-center justify-between px-3 py-2 rounded-xl ${bgColorClass} border border-white/50 shadow-sm`}>
                      ...
                      <span className={`text-[8px] font-black px-1.5 py-0.5 rounded-md bg-white/60 ${textColorClass}`}>
                  ...
                  <Droppable droppableId={col.id}>
                    {(provided) => (
                      <div
                        ...
                        className={`flex-1 space-y-2.5 min-h-[600px] p-2 rounded-xl ${bgColorClass} bg-opacity-40 border border-transparent`}
                      >
                              ...
                              {(provided) => (
                                ...
                                >
                                  <div
                                    ...
                                    className={`bg-white ${currentStyles.padding} rounded-xl border border-gray-100 shadow-sm hover:shadow-md transition-all relative overflow-hidden group cursor-pointer`}
                                  >
                                    <button
                                      onClick={(e) => {
                                        e.stopPropagation()
                                        ...
                                        <div className="relative group">
                                          <div className={`absolute inset-0 rounded-md transition-all duration-500 ${
                                            ticket.percentuale_avanzamento > 0
                                              ? 'bg-emerald-400/20 scale-105 blur-sm'
                                              : 'bg-transparent'
                                          }`} />
                                    ...
                                    <div className="flex items-center justify-between pt-2 border-t border-gray-50">
                                      ...
                                      <button
                                        ...
                                        className="p-1 bg-gray-900 text-white rounded hover:bg-blue-600 transition-all"
                                      >
      ...
      <div className="fixed bottom-6 left-1/2 -translate-x-1/2 z-50">
        <div className="flex items-center gap-2 bg-gray-900/90 backdrop-blur-md p-2 rounded-2xl shadow-2xl border border-white/10">
          <button
            ...
            className="p-2 text-white hover:bg-white/10 rounded-xl transition-colors disabled:opacity-30"
          ...
          <div className="flex items-center gap-1 px-3 border-x border-white/10">
            {[1, 2, 3, 4, 5].map((lvl) => (
              <div
                key={lvl}
                className={`w-1.5 h-1.5 rounded-full transition-all ${lvl <= zoomLevel ? 'bg-blue-500 scale-125' : 'bg-white/20'}`}
              />
            ))}
          </div>
